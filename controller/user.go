...
func HandlerLogin(c *gin.Context) {
	...
	//info := model.GetUserInfo("12345")
	name := c.Request.FormValue("name")
	value := c.Request.FormValue("pwd")
	...
	LoginSucceed(string(marshal), "12345", c)
...
func LoginByPwd(c *gin.Context)  {
	...
	if len(userInfo.UserName) ==0 {
		fmt.Println("用户不存在")
		c.JSON(http.StatusFound,gin.H{"data":"用户不存在,请注册"})
		return
	}
	user := &QUserInfo{
		Nickname:    userInfo.UserName,
		FigureUrlQQ: userInfo.ImagePath,
	}
	marshal, _ := json.Marshal(user)
	LoginSucceed(string(marshal), userInfo.OpenId, c)
...
func Register(c *gin.Context)  {
	...
	// 生成openID
	openID, _ := uuid.NewUUID()
	log.Println("userName:",userName,";pwd:",pwd,"openID:",openID)
	model.CreateUser(openID.String(),userName,pwd,"")
